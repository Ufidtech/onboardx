...
export default function AdminView() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(collection(db, 'onboardingStatus'), (snapshot) => {
  ...
  return (
    <div className="max-w-2xl mx-auto mt-12 px-4">
      <Card>
        ...
        <h1 className="text-lg font-medium mb-4">Onboarding, this cohort</h1>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-gray-100">
                ...
                <td className="py-2">{r.mentorName || <span className="text-gray-400">Pending</span>}</td>
                <td className="py-2">
                  <span className={`px-2 py-0.5 rounded-md text-xs ${STATUS_STYLES[r.status] || ''}`}>
            ...
            {rows.length === 0 && (
              <tr><td colSpan={3} className="py-4 text-center text-gray-400">No members yet.</td></tr>
            )}
          </tbody>
        </table>
      </Card>
